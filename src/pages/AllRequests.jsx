...
function AllRequests() {
  ...
  const filteredRequests = leaveRequests.filter((r) => {
    const matchesSearch = r.studentName.toLowerCase().includes(search.toLowerCase()) || r.department.toLowerCase().includes(search.toLowerCase()) || r.reason.toLowerCase().includes(search.toLowerCase());
  ...
  return <DashboardLayout><div className="flex items-center justify-between mb-6"><div><h1 className="text-2xl font-bold">All Leave Requests</h1><p className="text-sm text-muted-foreground">View and monitor all leave requests</p></div>{(user?.role === "admin" || user?.role === "principal") && <Button onClick={handleExportCSV} disabled={exporting} variant="outline" size="sm" className="gap-1.5"><Download className="h-4 w-4" />{exporting ? "Exporting..." : "Export CSV"}</Button>}</div><Card><CardHeader className="pb-3"><div className="flex items-center justify-between gap-3"><div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" /><Input
